...
@implementation SyncedPhotos (OpenPhoto)
...
+ (NSMutableArray *) getPathsInManagedObjectContext:(NSManagedObjectContext *)context;
{
    ...
    // with all paths
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"SyncedPhotos"];
    
    NSError *error = nil;
    NSArray *matches = [context executeFetchRequest:request error:&error];
    
    if (error){
        NSLog(@"Error to get all paths from SyncedPhotos on managed object context = %@",[error localizedDescription]);
        return nil;
    }
    
    for (SyncedPhotos *model in matches) {
